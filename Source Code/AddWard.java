...
public class AddWard extends JFrame implements ActionListener
{
	...
	JTextField ltf1,ltf2,ltf3,ltf4,ltf5,ltf6;
	...
	public AddWard()
	{
		...
		update.setBounds(270,340,130,30);
		p.add(update);		


		clear = new JButton("Clear",new ImageIcon("image/reset.png"));
		clear.setBounds(440,340,130,30);
		p.add(clear);
		...
		submit = new JButton("Add Ward",new ImageIcon("image/save.png"));
		submit.addActionListener(this);
		submit.setBounds(100,340,130,30);
		p.add(submit);
		
		delete = new JButton("Delete",new ImageIcon("image/cancel.png"));
		delete.addActionListener(this);
		delete.setBounds(610,340,130,30);
		p.add(delete);
		JLabel im=new JLabel(new ImageIcon("image/m1.jpg"));
		im.setBounds(0,0,850,450);
		p.add(im);
		add(p);
	...
	String 	s1,s2,s3,s4,s5,s6; re=0;  
					s1=ltf1.getText(); s2=ltf2.getText(); s3=ltf3.getText();  s4=ltf4.getText();  s5=ltf5.getText(); s6=ltf6.getText();
				...
				Connection conn = DriverManager.getConnection("jdbc:oracle:thin:@localhost:1521:xe",username,password);
      				String query = "delete from ward where roomno = ?";
      				...
      				conn.close();
						...
						rs2=stm2.executeQuery("Select * from ward");
							while (rs2.next())
							{
					...
					for(int y4=0;y4<i4.length(); ++y4)
					{
							...
							if(pass1==0)
						{
						   ...
						   try        { Class.forName("oracle.jdbc.driver.OracleDriver");
							con = DriverManager.getConnection("jdbc:oracle:thin:@localhost:1521:xe",username,password);
							stm =  con.prepareStatement("insert into ward values(?,?,?,?,?,?)");
							...
							stm.executeUpdate();
							JOptionPane.showMessageDialog(this,"Add Ward Successful","Success",JOptionPane.INFORMATION_MESSAGE);
							ltf1.setText("");	
				 			ltf2.setText("");
							ltf3.setText("");
							ltf4.setText("");
						...
						if(ph3==1) {JOptionPane.showMessageDialog(this,"Incorrect Bed no","Error",JOptionPane.ERROR_MESSAGE);}
						if(ph4==1) {JOptionPane.showMessageDialog(this,"Incorrect  Price","Error",JOptionPane.ERROR_MESSAGE);}
						}
						}else{JOptionPane.showMessageDialog(this,"Room no already add","Error",JOptionPane.ERROR_MESSAGE); }
						}
			...
			if(ea.getSource()==update)
				{   
			 		up=JOptionPane.showInputDialog("Enter Room No "); 
					...
					con = DriverManager.getConnection("jdbc:oracle:thin:@localhost:1521:xe",username,password);
					stm2=con.createStatement();
					rs2= stm2.executeQuery("SELECT  *  FROM ward WHERE roomno="+up+" ");
				...
				{	String ss=JOptionPane.showInputDialog("Enter Room No");
					re1=0;
					try{
						Class.forName("oracle.jdbc.driver.OracleDriver");
			                               		con = DriverManager.getConnection("jdbc:oracle:thin:@localhost:1521:xe",username,password);
				...
				Connection conn = DriverManager.getConnection("jdbc:oracle:thin:@localhost:1521:xe",username,password);
      				...
      				preparedStmt.execute();
				JOptionPane.showMessageDialog(this,"Delete Ward Successful","Success",JOptionPane.INFORMATION_MESSAGE);
      				conn.close();
				   }
